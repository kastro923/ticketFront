...
(function () {
    ...
    var SearchPane = /** @class */ (function () {
        ...
        SearchPane.prototype._reloadSelect = function (loadedFilter) {
            ...
            var idx;
            // For each pane, check that the loadedFilter ListTicket exists and is not null,
    ...
    var SearchPanes = /** @class */ (function () {
            ...
            if (!this.s.updating && !this.s.dt.page.info().serverSide) {
                ...
                if (!this.regenerating) {
                    for (var _f = 0, _g = this.s.panes; _f < _g.length; _f++) {
                        var pane = _g[_f];
                        // Identify the pane where a selection or deselection has been made and add it to the ListTicket.
                    ...
                    // Remove selections from the ListTicket from the pane where a deselect has taken place
                    for (var i = 0; i < this.s.selectionList.length; i++) {
                        if (this.s.selectionList[i].index !== deselectIdx || this.s.selectionList[i].protect === true) {
                            var further = false;
                            // Find out if this selection is the last one in the ListTicket for that pane
                            for (var j = i + 1; j < this.s.selectionList.length; j++) {
                                if (this.s.selectionList[j].index === this.s.selectionList[i].index) {
                                    further = true;
                                }
                            }
                            // If there are no selections for this pane in the ListTicket then just push this one
                            if (!further) {
         ...
         *
         * @param newSelectionList the ListTicket of selections which are to be made
         */
        SearchPanes.prototype._cascadeRegen = function (newSelectionList, selectTotal) {
            ...
            // Set the selection ListTicket property to be the ListTicket without the selections from the deselect pane
        ...
        /**
         * Gets the selection ListTicket from the previous state and stores it in the selectionList Property
         */
         ...
         *
         * @param newSelectionList the ListTicket of selections to be made, in the order they were originally selected
         */
        ...
        SearchPanes.prototype._paneDeclare = function (table, paneSettings, opts) {
            ...
            if (this.s.dt.settings()[0]._bInitComplete) {
                ...
                // Otherwise add the paneStartup function to the ListTicket of functions
        ...
        SearchPanes.prototype._serverTotals = function () {
            ...
            // Build an updated ListTicket based on any selections or deselections added
            if (!selectPresent) {
                this.s.selectionList = [];
            }
            else {
        ...
        SearchPanes.prototype._startup = function (table) {
            ...
            // Whenever a state save occurs store the selection ListTicket in the state object
            this.s.dt.on('stateSaveParams.dtsp', function (e, settings, data) {
                if (data.searchPanes === undefined) {
                    data.searchPanes = {};
                }
